...
| `BlobGrowth.cs` | Smooth scale formülü; tier hesabı; `PunchScale()` yeme feedback'i; B16: `_massGainMultiplier` (Sindirim skill'i) — `AddMass()`'ta hem mass hem XP'ye uygulanır (mass=XP birleşik, Karar 7) |
...
- **`BlobGrowth.CurrentMass`/`CurrentTier` restart'ta hiç sıfırlanmıyor (2026-08-22, M25 sırasında fark edildi — yukarıdaki maddeden ayrı, daha temel bir eksik):** Yukarıdaki restart maddesi `BlobHealth`/`WeaponBase`/`ScoreSystem`'i listeliyor ama `BlobGrowth` hiç yoktu — `GameManager`/`GameOverScreen.Restart()` içinde `BlobGrowth`'a dokunan hiçbir kod yok, `BlobGrowth.cs`'te de bir reset metodu hiç tanımlı değil. Sahne reload olmadığı için `CurrentMass`/`CurrentTier` bir önceki run'ın bitişindeki değerde kalıyor gibi görünüyor — teyit edilmedi (muhtemelen şimdiye kadar fark edilmemesinin sebebi test oturumlarının genelde Unity'yi yeniden açarak yapılması, gerçek in-session restart'ın az test edilmiş olması). Bir sonraki Play Mode oturumunda restart deneyip `CurrentTier`'ın gerçekten Tiny'ye dönüp dönmediği kontrol edilmeli; dönmüyorsa `BlobGrowth`'a bir `ResetGrowth()` eklenip restart akışına bağlanmalı.
