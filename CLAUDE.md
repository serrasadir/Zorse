...
| Script | Açıklama |
|--------|----------|
| `PedestrianController.cs` | `ConsumableBase`'i miras alır (IConsumable/layer/pool-return hazır gelir) — üzerine wander (rastgele waypoint, `PatrolState` deseni) + kaçış (`_blobGrowth.CurrentTier >= RequiredTier` VE blob `FleeTriggerDistance` içindeyse) + animasyonsuz sinüs-bob (`Cannon` mermisinin yay hareketiyle aynı teknik) ekler. `SwarmSteering` **kullanmaz** — o `EnemyBase`'e sıkı bağlı (dev-a dosyasına dokunmadan genelleştirilemezdi) ve yayaların ayrıca birbirinden kaçınmaya ihtiyacı yok. AI throttle 0.4s (staggered) |
| `PedestrianSpawner.cs` | `ConsumableSpawner`'ın pool/spawn-radius deseniyle aynı; `EnemySpawner`'ın canlı sayacına bağımlı olmadan (cross-file coupling riski) kendi sabit tavanıyla çalışır (`_maxActive=20`, sürü düşman tavanının belirgin altında — "aynı bütçeyi paylaşır" ilkesi burada dinamik koordinasyon yerine baştan küçük tutulan bir payla karşılanıyor) |
...
| `LobbyPanel.cs` | Oyun başında görünür, HUD'u gizler (`_hud.SetActive(false)`); 3 karakter butonu (icon + isim); tıklayınca paneli kapatır, HUD'u açar, `GameManager.Instance.StartGame(data)` çağırır; M23: `_marketButton` → `OpenMarket()` (kendi panelini gizler, `MarketPanel.Show()` çağırır); public `Show()` — `MarketPanel`'in "geri" butonu Inspector'dan buna bağlanacak (döngüsel script referansı yok, iki OnClick listener) |
...
- **A14 FPS/Profiler doğrulaması yapılmadı:** Kod tarafı (throttle, komşu sayısı sınırı, elit tavanı) tamamlandı ama 150-200 eşzamanlı düşman senaryosunda gerçek FPS ölçümü Unity Editor/cihaz gerektiriyor, buradan yapılamadı. Test için: Window → Analysis → Profiler (CPU), ya da Game view Stats overlay; hedef CLAUDE.md'deki "30 FPS @ 720p, 200 eşzamanlı düşman". **2026-08-20: Serra ile birlikte ertelendi** — Serra'nın Mac'i (M5) Editor'da Play Mode sırasında ısındığını fark etti; muhtemelen büyük ölçüde Editor overhead'i (Scene view render'ı + asset watching + arka plan derleme aynı anda), gerçek oyun performansından ayrıştırmak için macOS standalone Development Build + Autoconnect Profiler önerildi. Profiler kaydı alınıp birlikte yorumlanması bir sonraki oturuma bırakıldı — unutulmamalı.
- **Cannon (Topik) mermi arc/isabet düzeltmesi Play Mode'da test edilmedi:** 2026-08-20'de kod düzeltildi (bkz. Önemli Kararlar) + `Cannon.prefab`'ın `Arc Height`'ini `1.2`'den ~`0.4`'e düşürme önerisi verildi (henüz uygulanmadı) — ikisi de A14 performans testiyle birlikte bir sonraki oturumda doğrulanacak.
- **M20 (Sprint 5) Unity'de açılıp derleme doğrulaması bekliyor:** `MetaProgression.cs` + `.meta`'sı Unity kapalıyken elle eklendi, sahnedeki `MetaSystems` GameObject'ine üçüncü component olarak elle wired edildi (bkz. Önemli Kararlar). İlk açılışta compile hatası yoksa ve `MetaSystems` Inspector'da `Meta Progression` component'i doğru göründüyse sorun yok demektir — ekstra bir Editor adımı gerekmiyor, sadece doğrulama.
- **M21/M23 (Sprint 5) Editor kurulumu bekliyor — Market UI:** Kod tamam (`MarketPanel.cs` + `LobbyPanel.cs`'e eklenen `_marketButton`/`_marketPanel`), ama Canvas hiyerarşisi hiç yok. Gerekli adımlar: (1) `Canvas`'a (mevcut HUD/UpgradePanel/GameOverScreen'in durduğu yer) yeni bir `MarketPanel` GameObject'i ekle (başta inaktif), içine: kredi göstergesi (TMP_Text), 6 buton — her biri bir alt TMP_Text label'ı ile (Mıknato Karakteri, Hız, Max Can, Mass Kazanımı, Coin Kazanımı, XP Çarpanı sırasıyla — sıralama önemli, `_statTypes` dizisiyle index eşleşecek), bir de "Geri" butonu; (2) bu GameObject'e (ya da bir manager objesine) `MarketPanel` component'ini ekle, `_panel`→panelin kendisi, `_creditText`→kredi TMP_Text, `_statTypes`→6 elemanlı `[MiknatoCharacter, Speed, MaxHealth, MassGain, CoinGain, XpMultiplier]` (bu sırayla), `_buttons`→6 buton (aynı sıra), `_labelTexts`→6 TMP_Text (aynı sıra); (3) "Geri" butonunun OnClick listesine **iki** persistent call ekle: `MarketPanel.Hide` VE `LobbyPanel.Show` (döngüsel referans yok, ikisi ayrı ayrı Inspector'dan sürüklenip eklenecek); (4) `LobbyPanel` GameObject'inde Lobby'ye bir "Market" butonu ekle, `LobbyPanel` component'inin `_marketButton`'ına bu butonu, `_marketPanel`'ine (1)'de oluşturduğun `MarketPanel` component'ini ata. `MetaProgression`'ın `Instance`'ı zaten sahnede (M20'den, `MetaSystems` GameObject'i) — ek bir referans gerekmiyor, `MarketPanel` onu `MetaProgression.Instance` üzerinden buluyor.
- **M24 (Sprint 5) Editor kurulumu bekliyor:** Kod tamamlandı (bkz. Önemli Kararlar) ama B16/B17'nin aksine bu iş kalıcı asset/prefab gerektiriyor, tamamen kod tarafında bitmiyor. Gerekli adımlar: (1) en az 1 yaya görsel prefab'ı oluştur (basit mesh + `Collider` — **Is Trigger işaretli** olmalı, mevcut consumable prefab'larıyla aynı desen — + `PedestrianController` component'i; layer'ı elle ayarlamaya gerek yok, `ConsumableBase.OnEnable`/`AssignLayer()` `RequiredTier`'a göre otomatik atıyor); (2) en az 1 `PedestrianData.asset` oluştur (`Create → BlobSurvivor → Pedestrian Data`), `Prefab` alanına (1)'i, `Required Tier`'ı (örn. `Small` ya da `Medium` — hangi tier'da blob'un yayaları hem korkutup hem yiyebileceğine karar vermek dengeleme kararı, GDD net bir değer vermiyor) ata, hareket/bob değerlerini iste (varsayılanlar makul: MoveSpeed 1.2, FleeSpeed 3, WanderRadius 8, FleeTriggerDistance 6, BobHeight 0.08, BobSpeed 6); (3) sahneye `PedestrianSpawner` component'li bir GameObject ekle (`ConsumableSpawner`/`EnemySpawner` gibi sahnede duran bir singleton), `_pedestrianPool` dizisine (2)'yi ata. Physics Collision Matrix'te **yeni bir girdi gerekmiyor** — yayalar ConsumableTier1-5 layer'larını (Blob zaten bunlarla çarpışıyor) otomatik kullanıyor.
