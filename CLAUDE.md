...
| `PedestrianSpawner.cs` | `ConsumableSpawner`'ın pool/spawn-radius deseniyle aynı; `EnemySpawner`'ın canlı sayacına bağımlı olmadan (cross-file coupling riski) kendi sabit tavanıyla çalışır; 2026-08-31: `_maxActive` sahnede `25`'e ayarlı (GDD Karar 2 revizesi — bkz. Önemli Kararlar) |
...
| `EnemySpawner.cs` | NavMesh.SamplePosition ile Y=0.65f'te spawn; weighted random enemy seçimi; 2026-08-31: `_maxActiveEnemies` sabit `15` (eski tier-bazlı `40*tier+10, max 200` ölçekleme kaldırıldı — GDD Karar 2 revizesi, bkz. Önemli Kararlar), `_maxActiveElites` `8`→`4` |
