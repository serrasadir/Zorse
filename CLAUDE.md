...
- **Cannon (Topik) mermi arc/isabet düzeltmesi Play Mode'da hâlâ test edilmedi (GitHub #45):** 2026-08-20'de kod düzeltildi (bkz. Önemli Kararlar). **2026-08-31: `Cannon.prefab`'ın `Arc Height`'i `1.2`'den `0.4`'e düşürüldü** (Unity kapalı/açık bilinmeden, salt bir float değer değişimi olduğu için düşük riskli kabul edildi — Editor açıksa dosyayı reload etmesi gerekebilir). Play Mode doğrulaması hâlâ bekliyor.
...
- **2026-08-31 — Sprint 5 sonrası açık issue triyajı: #46 (Collision Matrix) doğrulandı + kapatıldı, #48 (Mıknato Lobby kilidi) implement edildi, #45 (Cannon arc height) değeri düşürüldü:** `CharacterData.cs`'e yeni `_requiresMarketUnlock` bool alanı eklendi (`Char_Miknato.asset`'te `1`), `LobbyPanel.cs`'e `RefreshCharacterButtons()` eklendi (`Start()` ve `Show()`'da çağrılır — Market'ten dönüşte kilit durumu güncel kalsın diye) — kilitliyken buton `interactable=false` + isim yanında "(Kilitli)". `Cannon.prefab`'ın `_arcHeight`'i `1.2`'den `0.4`'e düşürüldü (#45'in Editor önerisiydi, artık uygulandı). #46 için `ProjectSettings/DynamicsManager.asset`'teki collision matrix'i Unity açmadan hex-decode ederek Blob(8)×Enemy(14) bitinin işaretli olduğu doğrulandı (bkz. yukarıdaki Collision Matrix notu). #47 (karakter ikonları) ve #49 (repro bekleyen ölüm bug'ı) sanat/veri eksikliği nedeniyle koddan çözülemiyor, açık kaldı. #44 (FPS/Profiler) gerçek Editor/cihaz oturumu gerektirdiği için açık kaldı. **Unity açık/kapalı bilinmeden yazıldı — LobbyPanel/CharacterData derleme + Play Mode doğrulaması (kilitli buton görünümü, Market'ten satın alınca kilidin açılması) Serra tarafından yapılmalı.**
